...
impl<T> Merge<T> {
        ...
        T: PartialEq,
    {
        ...
        self.get_simplified_mapping_by(|original_indices, next_indices, add_index| {
            let mut remove_index = next_indices[0];
            while remove_index < unsimplified_len {
                if self.values[original_indices[remove_index]]
                    == self.values[original_indices[add_index]]
                {
                    return remove_index;
                }
                let following_add_index = next_indices[remove_index];
                remove_index = next_indices[following_add_index];
            }
            unsimplified_len
        })
    }

    fn get_simplified_mapping_with_hash(&self) -> Vec<usize>
    where
        T: Eq + Hash,
    {
        let unsimplified_len = self.values.len();
        ...
        for remove_index in (1..unsimplified_len).step_by(2) {
            remove_indices_by_value
                ...
                .or_default()
                .push_back(remove_index);
        }
        self.get_simplified_mapping_by(|original_indices, _next_indices, add_index| {
            remove_indices_by_value
                .get_mut(&self.values[original_indices[add_index]])
                .and_then(VecDeque::pop_front)
                .unwrap_or(unsimplified_len)
        })
    }

    fn get_simplified_mapping_by(
        &self,
        mut find_matching_remove: impl FnMut(&[usize], &[usize], usize) -> usize,
    ) -> Vec<usize> {
        let unsimplified_len = self.values.len();
        let mut original_indices = (0..unsimplified_len).collect_vec();
        // Keep the terms in a linked list so deleting a diff pair doesn't move all
        // following indices. In particular, repeatedly draining the Vec made
        // simplification quadratic in bytes copied for large merge expressions.
        let mut next_indices = (1..=unsimplified_len).collect_vec();
        let mut previous_indices = (0..unsimplified_len)
            .map(|index| index.saturating_sub(1))
            .collect_vec();

        let mut add_index = 0;
        while add_index < unsimplified_len {
            let matching_remove_index =
                find_matching_remove(&original_indices, &next_indices, add_index);

            if matching_remove_index < unsimplified_len {
                let following_add_index = next_indices[matching_remove_index];
                original_indices.swap(following_add_index, add_index);

                // Splice out the matching remove and the add immediately after it.
                let previous_index = previous_indices[matching_remove_index];
                let following_index = next_indices[following_add_index];
                next_indices[previous_index] = following_index;
                if following_index < unsimplified_len {
                    previous_indices[following_index] = previous_index;
                }

                if matching_remove_index < add_index {
                    // The deletion shifted the current logical position back by two,
                    // so continue at the next add to preserve the Vec algorithm's
                    // traversal order.
                    let following_remove_index = next_indices[add_index];
                    add_index = if following_remove_index < unsimplified_len {
                        next_indices[following_remove_index]
                    } else {
                        unsimplified_len
                    };
                }
        ...
        let mut simplified_to_original_indices = Vec::new();
        let mut index = 0;
        while index < unsimplified_len {
            simplified_to_original_indices.push(original_indices[index]);
    ...
    /// Simplifies the merge using a hash index to find canceling terms.
    #[must_use]
    pub(crate) fn simplify_with_hash(&self) -> Self
    where
        T: Eq + Hash + Clone,
    {
        let mapping = self.get_simplified_mapping_with_hash();
        self.apply_simplified_mapping(&mapping)
    }
    ...
    /// Simplifies the merge using a hash index and a projected comparison key.
    #[must_use]
    pub(crate) fn simplify_by_with_hash<'a, U>(&'a self, f: impl FnMut(&'a T) -> U) -> Self
    where
        T: Clone,
        U: Eq + Hash,
    {
        let mapping = self.map(f).get_simplified_mapping_with_hash();
        self.apply_simplified_mapping(&mapping)
    }
...
mod tests {
    ...
    fn test_merge_invariants() {
        fn check_invariants(terms: &[u32]) {
            ...
            assert_eq!(
                merge.get_simplified_mapping_with_hash(),
                merge.get_simplified_mapping(),
                "hash simplification changed the mapping for {merge:?}"
            );
            assert_eq!(
                merge.simplify_by_with_hash(|term| term),
                merge.simplify_by(|term| term),
                "hash simplify-by changed the result for {merge:?}"
            );
